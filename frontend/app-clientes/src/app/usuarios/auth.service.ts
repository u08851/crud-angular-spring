...
export class AuthService {
  ...
  login(usuario : Usuario): Observable<any>{
    ...
    const credenciales = btoa('angularapp' + ':' + '1234'); //btoa --> funcion para convertir a bs64
  ...
  obtenerDatosToken(accessToken:string) : any {
    if(accessToken != null){
      return JSON.parse(atob(accessToken.split('.')[1]));  // decodifcamos el token (para ver el contenido del  token)
    }
    return null;
  }
